# Curbox UIHider Script Syntax

UIHider lets you write small **scripts** that analyze an app's on‑screen UI and react to it:
draw overlays to hide elements, compute where to place them with math, or press back/home.
Where the [ViewBlocker](./view_blocker_rule.md) uses fixed `key:value` rules, UIHider gives you a
real (tiny) programming language — variables, math, loops, conditionals, and functions — so you can
express logic like *"cover everything between the stories row and the bottom nav, but nothing else."*

> **Who is this for?** Users comfortable with simple scripting. If you just want to hide a single
> element, the ViewBlocker is easier. Reach for UIHider when you need to *calculate* what to hide.
